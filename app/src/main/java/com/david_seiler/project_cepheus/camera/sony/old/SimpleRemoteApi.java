...
import com.david_seiler.project_cepheus.camera.sony.old.ServerDevice.ApiService;
import com.david_seiler.project_cepheus.camera.sony.utils.JsonResponseFix;
...
public class SimpleRemoteApi {
    ...
    public JSONObject getCameraMethodTypes() throws IOException {
        ...
        try {
            ...
            String responseJson = SimpleHttpClient.httpPost(url, requestJson.toString());
            log("Response: " + responseJson);
            responseJson = JsonResponseFix.methodTypesFix;
//            largeLog("the-error", "FULL RESPONSE?" + responseJson);
            return new JSONObject(responseJson);
        } catch (JSONException e) {
            throw new IOException(e);
        }
    ...
    public JSONObject getAvcontentMethodTypes() throws IOException {
        ...
        try {
            log("TESTS: getMethodTypes in getAvcontentMethodTypes");
            String url = findActionListUrl(service) + "/" + service;
            JSONObject requestJson =
                    new JSONObject().put("method", "getMethodTypes") //
                            .put("params", new JSONArray().put("")) //
